      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-3">
        {fallacies.length === 0 ? (
          ...
          fallacies.map((fallacy) => (
            ...
            >
              <span className="absolute right-2 top-2 border-2 border-ink bg-parchment px-2 py-[2px] font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-ink">
                FLAGGED
              </span>
              <p className="mb-1 font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-ink/70">
                [{formatTimestamp(fallacy.timestamp)}]
              </p>
              <p className="pr-20 font-mono text-xs font-bold uppercase tracking-[0.14em] text-accent">
                {fallacy.type}
              </p>
              <blockquote className="mt-2 border-l-[3px] border-accent pl-2 font-mono text-xs text-ink">
                "{fallacy.quote}"
              </blockquote>
